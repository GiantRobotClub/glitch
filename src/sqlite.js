  ...
  .then(async (dBase) => {
    ...
    try {
      ...
      if (!exists) {
        ...
        await db.run(
          "CREATE TABLE CardLocation (id INTEGER PRIMARY KEY AUTOINCREMENT, x integer, y integer, url TEXT)"
        );

        // Add default choices to table
        await db.run(
          "INSERT INTO CardLocation (x,y,url) VALUES " +
            "(20,20, 'https://images.squarespace-cdn.com/content/v1/5758783607eaa0b68fc52697/1626054816579-AQUEXF7R1C5FIK8SMGFH/mini_justica_snarl.png?format=300w')," +
            " (300,300, 'https://images.squarespace-cdn.com/content/v1/5758783607eaa0b68fc52697/1630029715711-UTT71IKGGN9S667RFCVX/mini-fmc.png?format=300w')," +
            " (900,900, 'https://images.squarespace-cdn.com/content/v1/5758783607eaa0b68fc52697/5541af4c-d397-4cdb-b689-5162ff2dcbe5/mini_you.png?format=300w')"
        );
        ...
        console.log(await db.all("SELECT * from CardLocation"));
...
module.exports = {
  ...
  getOptions: async () => {
    ...
    try {
      return await db.all("SELECT * from CardLocation");
    } catch (dbError) {
   ...
   */
  processMove: async (id, x, y) => {
    // Insert new Log table entry indicating the user choice and timestamp
    try {
      // Check the vote is valid
      const option = await db.all(
        "SELECT * from CardLocation WHERE id = ?",
        id
      );
      if (option.length > 0) {
        ...
        await db.run(
          "UPDATE CardLocation SET x = ?, y = ? WHERE id = ?",
          x,
          y,
          id
        );
  ...
  clearHistory: async () => {
    try {
      ...
      await db.run("UPDATE CardLocation SET x = 0, y = 0");
